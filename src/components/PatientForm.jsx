...
export default function PatientForm({ form, setForm, onChange, handleSubmit }) {
  // Filter sections dynamically based on resource type
  const filteredSections = React.useMemo(() => {
    switch (form.resourceType) {
      case "Observation":
        return fhirMultiSections.filter(s =>
          ["observations", "specimens"].includes(s.key)
        );
      case "Condition":
        return fhirMultiSections.filter(s =>
          ["conditions", "encounters", "goals"].includes(s.key)
        );
      case "Procedure":
        return fhirMultiSections.filter(s =>
          ["procedures", "devices", "serviceRequests"].includes(s.key)
        );
      case "Patient":
      default:
        return fhirMultiSections.filter(s=>
            ["allergies", "medications", "immunizations","relatedPersons"].includes(s.key)
        );
    }
  }, [form.resourceType]);
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="bg-gray-900/30 border border-gray-800 rounded-2xl p-4 sm:p-6 lg:p-8 space-y-6 backdrop-blur-sm">
        {/* Top row: resource type and status */}
        ...
        {/* Dynamically generated FHIR sections */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filteredSections.map(({ key, label, placeholder }) => (
            <MultiEntry
              key={key}
              label={label}
              value={form[key] || []}
